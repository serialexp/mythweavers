...
import { a, blockquote, doc, em, h1, li, p, ul } from './schema'
...
describe('Node', () => {
  describe('slice', () => {
    ...
    it('can cut to a deep position', () => {
      t(doc(blockquote(ul(li(p('a')), li(p('b<b>'))))), doc(blockquote(ul(li(p('a')), li(p('b'))))), 0, 4)
    })
    ...
    it('preserves styles after cut', () => {
      t(doc(p('a sentence with an ', em('emphasized ', a('li<a>nk')), ' in it')), doc(p(em(a('nk')), ' in it')), 1, 0)
    })

    it('preserves styles started after cut', () => {
      t(doc(p('a ', em('sentence'), ' wi<a>th ', em('text'), ' in it')), doc(p('th ', em('text'), ' in it')), 1, 0)
    })
    ...
    it('can cut from a deep position', () => {
      t(doc(blockquote(ul(li(p('a')), li(p('<a>b'))))), doc(blockquote(ul(li(p('b'))))), 4, 0)
    })
    ...
    it('can cut part of marked text', () => {
      t(doc(p("here's noth<a>ing and ", em("here's e<b>m"))), p('ing and ', em("here's e")), 0, 0)
    })
    ...
    it('can include parents', () => {
      ...
      expect(slice.toString()).toBe('<blockquote(paragraph("o"), paragraph("bar"))>(2,2)')
    })
  })
  ...
  describe('replace', () => {
    function rpl(
      ...
      const slice = insert ? insert.slice(insert.tag.a, insert.tag.b) : Slice.empty
      expect(eq(docNode.replace(docNode.tag.a, docNode.tag.b, slice), expected)).toBe(true)
    }
    ...
    it('can insert text', () => {
      rpl(doc(p('before'), p('on<a><b>e'), p('after')), doc(p('<a>H<b>')), doc(p('before'), p('onHe'), p('after')))
    })

    it("doesn't merge non-matching blocks", () => {
      rpl(doc(p('on<a>e'), p('t<b>wo')), doc(h1('<a>H<b>')), doc(p('onHwo')))
    })
    ...
    it('can replace within a block', () => {
      rpl(doc(blockquote(p('a<a>bc<b>d'))), doc(p('x<a>y<b>z')), doc(blockquote(p('ayd'))))
    })

    it('can insert a split', () => {
      rpl(doc(p('foo<a><b>bar')), doc(p('<a>x'), p('y<b>')), doc(p('foox'), p('ybar')))
    })

    it('keeps the node type of the left node', () => {
      rpl(doc(h1('foo<a>bar'), '<b>'), doc(p('foo<a>baz'), '<b>'), doc(h1('foobaz')))
    })

    it('keeps the node type even when empty', () => {
      rpl(doc(h1('<a>bar'), '<b>'), doc(p('foo<a>baz'), '<b>'), doc(h1('baz')))
    })
    ...
    function bad(
      ...
      const slice = insert ? insert.slice(insert.tag.a, insert.tag.b) : Slice.empty
      expect(() => docNode.replace(docNode.tag.a, docNode.tag.b, slice)).toThrow(new RegExp(pattern, 'i'))
    }
    ...
    it('rejects an unjoinable delete', () => {
      bad(doc(blockquote(p('a'), '<a>'), ul('<b>', li(p('b')))), null, 'cannot join')
    })

    it('checks content validity', () => {
      bad(doc(blockquote('<a>', p('hi')), '<b>'), doc(blockquote('hi', '<a>'), '<b>'), 'invalid content')
    })
  })
})
