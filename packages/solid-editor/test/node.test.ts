...
import { Fragment, Node, Schema } from '../src/model'
import { eq } from './builder'
import { a, blockquote, br, code, doc, em, hr, img, li, p, schema, strong, ul } from './schema'

// Custom schema for specific tests
const _customSchema = new Schema({
  nodes: {
...
describe('Node', () => {
  describe('toString', () => {
    ...
    it('shows inline children', () => {
      expect(doc(p('foo', img(), br(), 'bar')).toString()).toBe('doc(paragraph("foo", image, hard_break, "bar"))')
    })
  ...
  describe('cut', () => {
    ...
    it('extracts a full block', () => {
      testCut(doc(p('foo'), '<a>', p('bar'), '<b>', p('baz')), doc(p('bar')))
    })

    it('cuts text', () => {
      testCut(doc(p('0'), p('foo<a>bar<b>baz'), p('2')), doc(p('bar')))
    })
    ...
    it('works from the left', () => {
      testCut(doc(blockquote(p('foo<b>bar'))), doc(blockquote(p('foo'))))
    })

    it('works to the right', () => {
      testCut(doc(blockquote(p('foo<a>bar'))), doc(blockquote(p('bar'))))
    })
  ...
  describe('between', () => {
    function testBetween(doc: Node & { tag: { a: number; b: number } }, ...nodeNames: string[]) {
      ...
      doc.nodesBetween(doc.tag.a, doc.tag.b, (node, pos) => {
        ...
        if (compare !== nodeNames[i]) {
          throw new Error(`Expected ${JSON.stringify(nodeNames[i])}, got ${JSON.stringify(compare)}`)
        }
        i++
    ...
    it('iterates over text', () => {
      testBetween(doc(p('foo<a>bar<b>baz')), 'paragraph', 'foobarbaz')
    })
  ...
  describe('from', () => {
    function testFrom(arg: Node | Node[] | Fragment | null, expected: Node) {
    ...
    it('wraps an array', () => {
      testFrom([schema.node('hard_break'), schema.text('foo')], p(br(), 'foo'))
    })
  ...
  describe('toJSON', () => {
    ...
    it('can serialize nested nodes', () => {
      roundTrip(doc(blockquote(ul(li(p('a'), p('b')), li(p(img())))), p('c')))
    })
  })
})
