import { type Attrs, MarkType, Node, NodeType, Schema } from '../src/model'
...
function flatten(schema: Schema, children: ChildSpec[], f: (node: Node) => Node): { nodes: Node[]; tag: Tags } {
  const result: Node[] = []
  let pos = 0
  let tag: Tags = noTag
    ...
    if (typeof child === 'string') {
      ...
      if (childWithTag.tag && childWithTag.tag !== noTag) {
        ...
        for (const id in childWithTag.tag) {
          tag[id] = childWithTag.tag[id] + (childWithTag.flat || (child as Node).isText ? 0 : 1) + pos
        }
      }
...
function takeAttrs(attrs: Attrs | null, args: [a?: Attrs | ChildSpec, ...b: ChildSpec[]]): Attrs | null {
  const a0 = args[0]
  if (!args.length || (a0 && (typeof a0 === 'string' || a0 instanceof Node || (a0 as { flat?: unknown }).flat))) {
    return attrs
  }
...
function mark(type: MarkType, attrs: Attrs | null): MarkBuilder {
  return (...args: unknown[]) => {
    const markInstance = type.create(takeAttrs(attrs, args as [Attrs | ChildSpec, ...ChildSpec[]]))
