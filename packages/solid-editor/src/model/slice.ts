 ...
 * both sides are 'open' (cut through).
 */
export class Slice {
  /**
   * Create a slice. When specifying a non-zero open depth, you must
   * make sure that there are nodes of at least that depth at the
   * appropriate side of the fragment.
   */
  constructor(
    /** The slice's content. */
  ...
  insertAt(pos: number, fragment: Fragment): Slice | null {
    ...
    return content ? new Slice(content, this.openStart, this.openEnd) : null
  }
  ...
  toJSON(): { content?: ReturnType<Fragment['toJSON']>; openStart?: number; openEnd?: number } | null {
    if (!this.content.size) return null
    const json: { content: ReturnType<Fragment['toJSON']>; openStart?: number; openEnd?: number } = {
  ...
  static fromJSON(schema: Schema, json: { content?: unknown; openStart?: number; openEnd?: number } | null): Slice {
...
function insertInto(content: Fragment, dist: number, insert: Fragment, parent?: Node | null): Fragment | null {
  ...
  return inner ? content.replaceChild(index, child!.copy(inner)) : null
}

/**
 * Replace content between two resolved positions.
 */
export function replace($from: ResolvedPos, $to: ResolvedPos, slice: Slice): Node {
...
function addNode(child: Node, target: Node[]): void {
