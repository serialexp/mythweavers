...
export class Fragment {
  ...
  nodesBetween(
    from: number,
    to: number,
    f: (node: Node, start: number, parent: Node | null, index: number) => boolean | undefined,
    nodeStart = 0,
    parent?: Node,
  ): void {
   ...
   */
  descendants(f: (node: Node, pos: number, parent: Node | null, index: number) => boolean | undefined): void {
  ...
  addToStart(node: Node): Fragment {
    return new Fragment([node].concat(this.content as Node[]), this.size + node.nodeSize)
  }

  /** Create a new fragment by appending the given node. */
  addToEnd(node: Node): Fragment {
    return new Fragment((this.content as Node[]).concat(node), this.size + node.nodeSize)
  }
  ...
  static from(nodes?: Fragment | Node | readonly Node[] | null): Fragment {
    ...
    if (Array.isArray(nodes)) return Fragment.fromArray(nodes)
    if ((nodes as Node).attrs !== undefined) return new Fragment([nodes as Node], (nodes as Node).nodeSize)
