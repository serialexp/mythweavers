...
import type { ContentMatch, MarkType, NodeType, Schema } from './schema'
...
export class Node {
  ...
  nodesBetween(
    from: number,
    to: number,
    f: (node: Node, pos: number, parent: Node | null, index: number) => undefined | boolean,
    startPos = 0,
  ): void {
  ...
  descendants(f: (node: Node, pos: number, parent: Node | null, index: number) => undefined | boolean): void {
  ...
  /**
   * Test whether the content of another node could be appended to this node.
   */
  ...
  /**
   * Test whether replacing the range from `from` to `to` (by child index)
  ...
  canReplaceWith(from: number, to: number, type: NodeType, marks?: readonly Mark[]): boolean {
    return this.type.canReplaceWith(from, to, type, marks)
  }

  /**
   * Get the content match at the given index.
   */
  contentMatchAt(index: number): ContentMatch {
    return this.type.contentMatchAt(index)
  }
