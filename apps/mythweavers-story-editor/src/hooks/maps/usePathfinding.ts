...
import { Fleet, Hyperlane, Landmark } from '../../types/core'
import { getFleetPositionAtTime } from '../../utils/fleetUtils'
import { PathSegment, calculateOptimalPath, formatTravelTime } from '../../utils/maps/pathfinding'
...
export interface UsePathfindingReturn {
  showPathPreview: (
    ...
    targetX: number,
    targetY: number,
  ) => { travelTime: number; segments: PathSegment[] } | null
  ...
  const showPathPreview = (fleet: Fleet, targetX: number, targetY: number) => {
    ...
    for (const segment of result.segments) {
      ...
      if (segment.type === 'hyperlane') {
        ...
        drawDashedLine(previewGraphics, startWorldX, startWorldY, endWorldX, endWorldY, 10, 5, 0xffff00, 0.6)
      }
    }
