 ...
 */
export function createFleetSprite(fleet: Fleet, options: FleetRenderOptions): FleetSprite {
  ...
  if (variant === 'military') {
    ...
    sprite.moveTo(Math.cos(rotation) * starRadius, Math.sin(rotation) * starRadius)
    for (let i = 0; i < 10; i++) {
  ...
  const indicatorColor = options.position.status === 'in_transit' ? 0xffaa00 : 0x00ff00
 ...
 */
export function positionFleet(sprite: FleetSprite, position: FleetPosition, mapWidth: number, mapHeight: number): void {
 ...
 */
export function updateFleetStatus(sprite: FleetSprite, status: 'stationed' | 'in_transit'): void {
  if (sprite.statusIndicator) {
    const indicatorColor = status === 'in_transit' ? 0xffaa00 : 0x00ff00
    sprite.statusIndicator.clear()
    sprite.statusIndicator.circle(0, (SIZE_MAP[sprite.fleetData?.size || 'medium'] * 1.5) / 2 + 5, 3)
