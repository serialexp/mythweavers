...
export function drawDistanceField(
  ...
  console.log(
    `Distance field: ${width}x${height} grid (${width * height} cells), influence radius: ${influenceRadius.toFixed(0)}px, resolution: ${resolution}px`,
  )
  ...
  const renderChunk = () => {
    ...
    while (currentRow < height) {
      ...
      for (let x = 0; x < width; x++) {
        ...
        let totalWeight = 0
        let minDistance = Number.POSITIVE_INFINITY // Track closest landmark for opacity
        ...
        for (const landmark of landmarks) {
          ...
          if (distance <= influenceRadius * 2) {
            ...
            const weight = (1 - normalizedDist) ** 6 // Power of 6 for well-defined territories
        ...
        if (x === Math.floor(width / 2) && y % 20 === 0) {
          const zone = minDistance <= influenceRadius ? 'INNER' : minDistance <= influenceRadius * 2 ? 'FADE' : 'OUT'
          if (zone === 'FADE') {
            console.log(
              `Row ${y}: zone=${zone}, influences=${influences.length}, totalWeight=${totalWeight.toFixed(3)}, color=#${blendedColor.toString(16).padStart(6, '0')}, r=${r.toFixed(0)}, g=${g.toFixed(0)}, b=${b.toFixed(0)}, alpha=${alpha.toFixed(2)}`,
            )
          }
        }
