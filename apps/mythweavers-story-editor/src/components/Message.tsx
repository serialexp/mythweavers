...
export const Message: Component<MessageProps> = (props) => {
  ...
  const messageCost = createMemo(() => {
    ...
    const cacheReadSavings =
      tokenUsage.input_cache_read > 0 ? tokenUsage.input_cache_read * (promptPrice - cacheReadPrice) : 0
  ...
  const canRegenerateFromHere = () => {
    return (
      ...
      !props.message.isQuery &&
      messagesStore.canRegenerateFromMessage(props.message.id)
    )
  }
  ...
  const getMessageClasses = (isInstruction = false): string => {
    return cn(
      ...
      !messagesStore.isMessageActive(props.message.id) && styles.messageInactive,
    )
  }
  ...
  return (
    <>
      ...
      >
        <div class={getMessageClasses(true)} data-message-id={`${props.message.id}-instruction`}>
          ...
          <Show when={isEditingInstruction()}>
            <div
              class={cn(styles.content, styles.contentEditable)}
          ...
          <div class={styles.actions}>
            <div class={styles.timestamp}>
              ...
              <Show when={hasUnsavedChanges() && !isEditingInstruction()}>
                <span class={styles.draftSaved}>• Draft saved</span>
              </Show>
            </div>
      ...
      <div ref={messageRef} data-message-id={props.message.id} class={getMessageClasses()}>
        ...
        <Show when={props.message.script && props.message.role === 'assistant' && !props.message.isQuery}>
          {(() => {
            ...
            return shouldShow && dataState ? (
              <ScriptDataDiff before={dataState.before} after={dataState.after} messageId={props.message.id} />
            ) : null
          })()}
        </Show>
              ...
              {JSON.stringify(
                {
                  ...
                  sceneAnalysisEmpty: props.message.sceneAnalysis
                    ? Object.keys(props.message.sceneAnalysis).length === 0
                    : 'no sceneAnalysis',
        ...
        <div class={styles.actions}>
          <div class={styles.timestamp}>
            <span class={cn(styles.tokenInfo, styles.orderHighlight)}>Order: {props.message.order}</span>
            <Show when={props.message.role === 'assistant' && props.message.tokensPerSecond}>
              <span class={styles.tokenInfo}>{props.message.tokensPerSecond} tok/s</span>
            </Show>
            <Show when={props.message.isQuery}>
              <span class={styles.tokenInfo}>Query</span>
            </Show>
            <Show when={props.message.role === 'assistant' && getTokenUsage(props.message)}>
              {(() => {
                ...
                return (
                  <>
                    <span class={styles.tokenInfo}>{totalInput} pt</span>
                    <Show when={usage.input_cache_read > 0}>
                      <span class={cn(styles.tokenInfo, styles.cacheHit)}>{usage.input_cache_read} cached</span>
                    </Show>
                    <Show when={usage.input_cache_write > 0}>
                      <span class={styles.tokenInfo}>{usage.input_cache_write} cache write</span>
                    </Show>
                  </>
                )
              })()}
      ...
      <Show when={showScriptModal()}>
        <MessageScriptModal message={props.message} onClose={() => setShowScriptModal(false)} />
      </Show>
