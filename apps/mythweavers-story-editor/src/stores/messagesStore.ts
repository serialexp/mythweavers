...
saveService.setCallbacks({
  ...
  onOperationFailed: (operation, error) => {
    ...
    console.log(
      `[SaveService] Rolling back failed operation: ${operation.type} for ${operation.entityId}`,
      error.message,
    )
...
const triggerMessageSave = (
  ...
  saveService.saveMessage(currentStoryStore.id, messageId, message, operation, debounce, afterMessageId)
}
...
const totalStoryTurns = createMemo(() => {
  ...
  return visibleMessages().filter((msg) => msg.role === 'assistant' && !msg.isQuery).length
})
...
const activePath = createMemo(() => {
  ...
  if (messages.length === 0 || nodes.length === 0 || Object.keys(branchChoices).length === 0) {
    return {
...
export const messagesStore = {
  ...
  isMessageActive(messageId: string): boolean {
    ...
    if (activeIds.size === 0 && Object.keys(currentStoryStore.branchChoices || {}).length === 0) {
      return true
    }
  ...
  getStats: (charsPerToken: number, model?: string, provider?: 'ollama' | 'openrouter' | 'anthropic' | 'openai') => {
  ...
  getInsertAfterIdForNode: (nodeId: string): string | null => {
    console.log('[messagesStore.getInsertAfterIdForNode] Looking for insertion point for node:', nodeId)
    const messages = messagesState.messages
    ...
    console.log('[messagesStore.getInsertAfterIdForNode] No messages found in node')
  ...
  attachOrphanedMessagesToNode: (targetNodeId: string) => {
    ...
    if (storyId && currentStoryStore.storageMode === 'server' && updates.length > 0) {
  ...
  insertMessage: (afterMessageId: string | null, message: Message) => {
    ...
    triggerMessageSave(message.id, messageWithOrder, 'insert', false, afterMessageId)
  },
  ...
  updateMessage: (id: string, updates: Partial<Message>) => {
    ...
    if (index !== -1) {
      ...
      const currentOrder = updatedMessage.order !== undefined ? updatedMessage.order : index
  ...
  updateMessageNoSave: (id: string, updates: Partial<Message>) => {
    ...
    if (index !== -1) {
      ...
      // The conversion to Paragraph objects should happen here or this logic should be removed
      // Note: Auto-splitting content into paragraphs is disabled
      // The actual paragraph management should be done elsewhere with proper Paragraph structure
  ...
  setIsAnalyzing: (analyzing: boolean) => setMessagesState('isAnalyzing', analyzing),

  setShowStorageFullModal: (show: boolean) => setMessagesState('showStorageFullModal', show),

  setShowConflictDialog: (show: boolean) => setMessagesState('showConflictDialog', show),
  ...
  deleteMessage: (messageId: string) => {
    ...
    const messageIndex = messagesState.messages.findIndex((msg) => msg.id === messageId)
  ...
  deleteMessageNoSave: (messageId: string) => {
    ...
    setMessagesState('messages', (prev) => prev.filter((_, index) => index !== messageIndex))
  },

  // Move a message to a new position (after targetMessageId)
  moveMessage: async (messageId: string, targetMessageId: string | null, targetNodeId: string) => {
    ...
    if (!messageToMove) {
      console.error('[messagesStore.moveMessage] Message not found:', messageId)
      return
    }
    ...
    if (isMovingBetweenNodes && sourceNodeId) {
      const remainingSourceMessages = sourceNodeMessages.filter((m) => m.id !== messageId)
    ...
    let insertIndex = 0
    if (targetMessageId) {
      const targetIndex = targetNodeMessages.findIndex((m) => m.id === targetMessageId)
      if (targetIndex !== -1) {
  ...
  canRegenerate: () => {
    const lastMessage = messagesState.messages[messagesState.messages.length - 1]
    return (
      lastMessage &&
  ...
  canRegenerateFromMessage: (messageId: string) => {
    ...
    return !!message.instruction && !message.isQuery && !messagesState.isLoading
  },
  ...
  getMessagesNeedingAnalysis: () => {
    return messagesState.messages.filter(
      (msg) => msg.role === 'assistant' && !msg.isQuery && !msg.sceneAnalysis && msg.content.trim(),
    )
  },
  ...
  replaceInMessage: (
    ...
    for (const section of sections) {
      ...
      if (section === 'think' && message.think && message.think.includes(searchText)) {
  ...
  replaceAllInMessage: (
    ...
    for (const section of sections) {
      if (section === 'content' && message.content) {
        const matches = (
          message.content.match(new RegExp(searchText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []
        ).length
        if (matches > 0) {
      ...
      if (section === 'instruction' && message.instruction) {
        const matches = (
          message.instruction.match(new RegExp(searchText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []
        ).length
        if (matches > 0) {
          updates.instruction = message.instruction.split(searchText).join(replaceText)
          replacementCount += matches
        }
      }
      if (section === 'think' && message.think) {
        const matches = (message.think.match(new RegExp(searchText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || [])
          .length
        if (matches > 0) {
  ...
  cleanupThinkTags: () => {
    ...
    messagesState.messages.forEach((message, index) => {
      if (message.content?.includes('<think>') && message.content.includes('</think>')) {
        const { cleanedContent, thinkContent } = extractThinkTags(message.content)
  ...
  createEventMessage: (afterMessageId: string | null, content: string, script: string) => {
  ...
  createBranchMessage: (afterMessageId: string | null, content: string, nodeId?: string) => {
    ...
    const inserted = messagesState.messages.find((m) => m.id === branchMessage.id)
    console.log('[createBranchMessage] After insertion, order:', inserted?.order)
